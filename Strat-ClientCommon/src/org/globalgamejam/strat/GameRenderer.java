...
public class GameRenderer implements ApplicationListener {
	...
	public void render() {
		...
		if(selected != -1)
		{
			...
			cursor.draw(batch2);
			Gdx.app.log("lulz", "selected : " + selected + " select : " + select);
		}
		batch2.end();
	...
	private void touchProcessing() {
		...
		Gdx.app.log("setSelected", " new selection : " + selected);
		...
		{
			Gdx.app.log("action", "transfert de pierre du joueur vers " + select);
			com.giveStone(select);
		}
		else if(select == monId) // if the last selected avatar is the player's one
		{
			Gdx.app.log("action", "transfert de pierre vers le joueur de la part de " + select);
			com.stealStone(selected);
		}
		else
		{
			Gdx.app.log("action", "action spéciale de " + selected + "vers " + select);
		}
