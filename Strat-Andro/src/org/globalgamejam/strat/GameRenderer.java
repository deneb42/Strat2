...
public class GameRenderer implements ApplicationListener{
	...
	public void render() {
		...
		batch.begin();
			...
			for(i=0;i<nbBlock;i++)
			{
				blockBar[id].setPosition(10, 5 + i*(blockBar[id].getHeight()+5));
	...
	private void loadTextures() {
		...
		for(int i=0;i<NB_JOUEURS;i++)
			avatars[i] = new Sprite(new TextureRegion(texAvatar, (i%4)*128, (i/4)*128, 128, 128));

		for(int i=0;i<NB_BONUS;i++)
			bonus[i] = new Sprite(new TextureRegion(texBonus, (i%2)*64, (i/2)*64, 64, 64));
		
		for(int i=0;i<NB_SPRITE_LIFEBAR;i++)
			lifeBar[i] = new Sprite(new TextureRegion(texLifeBar, i*64, 0, 64, 32));
